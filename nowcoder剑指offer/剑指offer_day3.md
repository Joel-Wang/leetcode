...
* O( log10(n) )思路：依次计算个位、十位…一直到最高位的每个位置的1的个数，具体分为3种情况，以12x15百位x为例，此时base=100, 记a=n/base取整，b=n%base取整:

  >x为0：此时a=120,b=15，此时只考虑个十百前三位，百位为1的为100,101…199，共100个，而且随着万位千位百位从001，011,021,…,111，实际上1出现为12*100次；记为num[3]=a/10 *base = (a+8)/10 * base（为什么这么记可以看后边），b没有用；
  >
  >x为1：此时a=121,b=15，此时只考虑前三位，百位为1同上为100个（与base相同，这也是base的意义），随着万位千位百位从001,011,021...一直到111，实际上1出现的次数为12*100，记为a/10 *base ；对于为121_ _的数，百位1出现的次数为余数b加上1（即12100），因此，实际上出现的次数记为num[3]=(a+8)/10 * base+(b+1)
  >
  >x为2~9：此时a=12x，b=15，此时只考虑前三位，百位为1的为100个，此时12100~12199全部都已出现，因此实际上1出现的次数为num[3]=(a/10+1) *base = (a+8)/10 *base；（此时可以看出，a+8，实际上是为了满足0,1的时候不进位，2~9时进一位的需求）；


由以上可以看出，只有在对应位置为1的时候，需要计算b=n%base，通过对(a+8)/10计算，使得满足x为0,1,2~9三种情况，对(b+1)，令其与(a%10==1)相乘，来判断x是否为1；即可得到在三种情况下的表达式(base=1,10, ... ,base_max(为小于等于n的最大的（10^x，x为正整数）数）：
三种情况下分别表达式：
>a = n/base, b = n%base;
>
>> x == 0:  num[k] = a/10 *base;
>
>> x == 1:  num[k] = a/10 * base + (b+1);
>
>> x == 2~9 ：num[k] = (a/10+1) * base;

合并为一个通用表达式：

>a = n/base, b = n%base;
>
>num[k] = (a+8)/10 * base + (b+1)*(a%10==1);
